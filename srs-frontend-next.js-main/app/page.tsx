...
export default function LoginPage() {
  ...
  async function loginUser(email: string, password: string, role: UserRole) {
    try {
      ...
      console.error("Login failed:", err)
      // Set error message based on response or use a generic message
      setLoginError(err.response?.data?.message || "Login failed. Please check your credentials and try again.")
      return null
    } finally {
      setIsLoading(false)
    }
